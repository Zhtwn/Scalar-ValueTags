...
# removing value tags
{
    ...
    is( get_value_tags( $vt_type, \$var ), [ exact_ref($tag_one), exact_ref($tag_two), exact_ref($tag_three) ],
        'SETUP: get_value_tags should return all three tags' );

    # case 1: remove internal tag
    remove_value_tag( $vt_type, \$var, $tag_two );

    is( get_value_tags( $vt_type, \$var ), [ exact_ref($tag_one), exact_ref($tag_three) ],
